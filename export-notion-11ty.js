/* Notion → 11ty Markdown exporter (no hardcoded property names) */
...
function loadConfig() {
  ...
  for (const p of tryPaths) {
    if (fs.existsSync(p)) {
      if (p.endsWith(".json")) {
        ...
        // Lazy YAML support if installed; otherwise ignore
        try {
          const yaml = require("js-yaml");
          return yaml.load(fs.readFileSync(p, "utf8"));
        } catch {
          console.warn(
            "YAML config found but js-yaml not installed; ignoring:",
            p,
          );
        }
      }
  ...
  console.error("No notion.config.json found in project root");
  process.exit(1);
}
...
function getFirstTitleText(page) {
  // Find the first title-type property and return its plain text
...
function extractPropValue(prop) {
  // Convert any Notion property to a JSON-serializable plain value
  ...
  switch (prop.type) {
    ...
    case "date":
      // Prefer start; if end exists you can adapt to range
    ...
    case "formula":
      // Simplify common formula value types
    ...
    case "rollup":
      // Basic rollup handling: flatten array of values where possible
...
function normalizeRollup(rollup) {
  ...
  switch (rollup.type) {
    ...
    case "array":
      // Convert each element by best effort
      return (rollup.array || [])
        .map((el) => {
...
function detectDbConfig(dbConf, dbMeta) {
  if (!dbConf.srcDir) {
    console.error(`Missing required field 'srcDir' in database config`);
    process.exit(1);
  }
  if (!dbConf.basePath) {
    console.error(`Missing required field 'basePath' in database config`);
    process.exit(1);
  }
  if (!dbConf.layout) {
    console.error(`Missing required field 'layout' in database config`);
    process.exit(1);
  }
  ...
  // Default slug config if not provided
  const slugConf = dbConf.slug || {
    from: "title",
    fallback: "id",
    lower: true,
  };
  // Default permalink to basePath/{slug}/ if not provided
...
// ---------- Main ----------
async function fetchDatabaseMeta(database_id) {
  return notion.databases.retrieve({ database_id });
}

async function fetchAllPages(data_source_id) {
  const results = [];
  let hasMore = true;
...
async function pageBodyMarkdown(pageId, slug, imagesDir) {
  try {
    ...
    while ((match = imageRegex.exec(markdown)) !== null) {
      ...
      // Only process Notion URLs (they contain 'notion' or 'secure.notion-static.com')
      if (
...
async function writePage(dbCfg, page) {
  ...
  const body = await pageBodyMarkdown(page.id, slug, dbCfg.imagesDir);
...
async function run() {
  ...
  for (const dbConf of config.databases) {
    if (!dbConf.databaseId) {
      console.error("Each database config must have a 'databaseId' field");
      process.exit(1);
    }

    const dbId = dbConf.databaseId;
    const dbMeta = await fetchDatabaseMeta(dbId);
    ...
    if (!dbMeta.data_sources || dbMeta.data_sources.length === 0) {
      console.error(`No data sources found for database ID: ${dbId}`);
      process.exit(1);
    }
    ...
    for (const page of pages) {
      const outPath = await writePage(dbCfg, page);
